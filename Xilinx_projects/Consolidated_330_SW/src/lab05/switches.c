...
int switches_init(){
	...
	if(Xil_In32(XPAR_GPIO_SLIDE_SWITCHES_BASEADDR + TRI_STATE_OFFSET) == TRI_STATE_READ) {
...
void switches_runTest(){
	...
	int finish = 0;
	int32_t switch_status = 0x00;

	while(!finish){
		switch_status = switches_read();
		leds_write(switch_status);
		if(switch_status == 0x0f)
			{finish = 1;}
	}
}
