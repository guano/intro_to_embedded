...
void disp_button(char blank, char button) {
	switch(button){
	case BUTTON_0:
		disp_button_0(blank);	//display button 0
		break;
	case BUTTON_1:
		disp_button_1(blank);	//display button 1
		break;
	case BUTTON_2:
		disp_button_2(blank);	//display button 2
		break;
	case BUTTON_3:
		disp_button_3(blank);	//display button 3
		break;
	}
}
...
void disp_button_0(char blank) {
	...
	int h = display_height();	//the height
	int w = display_width();	//the width

	if(blank){
		//blank it with black! the right quarter of the screen
		display_fillRect(w*3/QUARTER_WIDTH,  0,  w,  h/HALF_SCREEN, DISPLAY_BLACK);
	}else {
		//draw it. the right quarter of the screen
		display_fillRect(w*3/QUARTER_WIDTH,  0,  w,  h/HALF_SCREEN, DISPLAY_YELLOW);
		display_setCursor(w*3/QUARTER_WIDTH,   h/CURSOR_HEIGHT);
		display_setTextColor(DISPLAY_CYAN);		//display the name of the button
...
void disp_button_1(char blank) {
	...
	int h = display_height();	//height
	int w = display_width();	//width

	if(blank){
		//blank it. the 3rd quarter
		display_fillRect(w/HALF_SCREEN,  0,  w/QUARTER_WIDTH,  h/HALF_SCREEN, DISPLAY_BLACK);
	}else {
		//draw it. the 3rd quarter
